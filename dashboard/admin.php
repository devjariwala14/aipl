...
if (isset($_REQUEST["btndelete"])) {
    ...
    try {
        $stmt_doc = $obj->con1->prepare("SELECT * FROM `admin` WHERE id=?");
        $stmt_doc->bind_param("i", $id);
        $stmt_doc->execute();
        $Resp_doc = $stmt_doc->get_result()->fetch_assoc();
        $stmt_doc->close();

        if (file_exists("images/admin/" . $Resp_doc["icon"])) {
            unlink("images/admin/" . $Resp_doc["icon"]);
            }

        $stmt_del = $obj->con1->prepare("DELETE FROM `admin` WHERE id=?");
            ...
            <div class="card">
                <div class="card-body">
                    ...
                    <table class="table datatable">
                        ...
                        <tbody>
                            <?php
                            $stmt_list = $obj->con1->prepare("SELECT * FROM `admin` ORDER BY `id` DESC;");
                            $stmt_list->execute();

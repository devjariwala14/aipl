...
if (isset($_REQUEST["btndelete"])) {
    ...
    try {
        $stmt_subimg = $obj->con1->prepare("SELECT * FROM `services` WHERE id=?");
        $stmt_subimg->bind_param("i", $s_id);
        $stmt_subimg->execute();
        $Resp_subimg = $stmt_subimg->get_result()->fetch_assoc();
        $stmt_subimg->close();

        if (file_exists("images/services/" . $Resp_subimg["image"])) {
            unlink("images/services/" . $Resp_subimg["image"]);
        }

        $stmt_del = $obj->con1->prepare("DELETE FROM `services` WHERE id=?");
                                ...
                                <tr>
                                    ...
                                    <td>
                                        ...
                                        if (in_array($extn, $img_array)) {
                                            ?>
                                            <img src="images/services/<?php echo $row["image"]; ?>" width="200" height="200"

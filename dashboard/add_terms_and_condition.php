...
if (isset($_COOKIE['edit_id'])) { $mode='edit' ; $editId=$_COOKIE['edit_id'];
    $stmt=$obj->con1->prepare("SELECT * FROM `terms_and_conditions` WHERE id=?");
    ...
    if (isset($_REQUEST["save"])) {
        ...
        try {
            // echo"INSERT INTO `terms_and_conditions`(`details`, `date_time`,`operation`) VALUES (".$details.", ".$date_time.",".$operation.")";
            $stmt = $obj->con1->prepare("INSERT INTO `terms_and_conditions`(`details`, `date_time`,`operation`) VALUES (?,?,?)");
        ...
        if ($Resp) {
            ...
            if (isset($_REQUEST["update"])) {
                ...
                try {
                    $stmt = $obj->con1->prepare("UPDATE `terms_and_conditions` SET `details`=?, `date_time`=?,`operation`=? WHERE `id`=?");
                ...
                <div class="card-body">
                    ...
                    <form class="row g-3 pt-3" method="post">
                        ...
                        <div class="text-left mt-4">
                            ...
                            <button type="button" class="btn btn-danger" onclick="window.location='terms_and_condition.php'">
                                Close</button>
                        </div>
                    </form>
                </div>
...
function go_back() {
    eraseCookie("edit_id");
    eraseCookie("view_id");
    window.location = "terms_and_condition.php";
}
</script>
<?php
        include "footer.php";
